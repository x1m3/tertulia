...
func TestTopicRepo(t *testing.T) {
	...
	availableRepos := make(map[string]repos.Itopics)
	availableRepos["memory"] = repos.NewTopicsMemory()

	db, err  := storm.Open("tertulia.db", storm.Batch(), storm.Codec(gob.Codec))
	if err !=nil {
		t.Fatal(err)
	}
	defer func() {
		db.Close()
		os.Remove("tertulia.db")

	}()
	availableRepos["storm"] = repos.NewTopicsStorm(db)


	for repoName, topicsRepo := range availableRepos {
